...
></iframe>
	...
	<div>
		...
		<p><b><u>Standard Metric</u></b>: For our player position classification task, we are using precision, recall, and F1-score as they give deeper insight into class-wise performance, while accuracy serves as an overall performance measure. Precision ensures that when the model predicts a role, it is likely correct, preventing frequent misclassification. Recall helps capture all instances of a given role, reducing false negatives. F1-score balances precision and recall, making it especially useful if class imbalances exist. Support provides context on class distribution, helping interpret other metrics. Accuracy, our standard metric, gives an overall measure of correct classifications. Together, these metrics offer a comprehensive evaluation of model performance beyond just accuracy, ensuring both correctness and completeness in role predictions. Metrics like log loss, AUC-ROC, and MSE are less suited since they either focus on probability estimation, binary classification, or regression, none of which directly align with our goal of predicting categorical roles..</p>
